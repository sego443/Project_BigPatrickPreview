...
def init_model(llm_config):
     tokenizer = AutoTokenizer.from_pretrained('./BigPatrickPreview_tokenizer')  # 加载分词器
     model = BigPatrickPreview(llm_config).to(args.device)  # 初始化模型并移动到设备
...
if __name__=='__main__':
     ...
     parser.add_argument("--use_wandb", action="store_true")  # 是否使用wandb进行日志记录
     parser.add_argument("--dtype", type=str, default="bfloat16")  # 数据类型，默认使用bfloat16
     parser.add_argument("--wandb_project", type=str, default="BigPatrickPreview-Pretrain")  # wandb项目名称
