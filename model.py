...
# Transformer层：包含注意力和前馈网络（这里称为BigPatrickPreviewBlock）
class BigPatrickPreviewBlock(nn.Module):
...
class BigPatrickPreview(PreTrainedModel):
     ...
     def __init__(self, params: LLMConfig = None):
          ...
          # 多层Transformer结构
          self.layers = nn.ModuleList([BigPatrickPreviewBlock(l, params) for l in range(self.n_layers)])
